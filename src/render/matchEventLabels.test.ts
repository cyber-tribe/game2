...
describe("describeMatchEvent", () => {
  it("describes a plain miracle event with the acting faction as subject", () => {
    ...
    expect(describeMatchEvent("armageddon", "enemy")).toBe("☠️ 敵が最終決戦を発動した");
  });
  ...
  it("describes houseReachedCastle by whose house it was, without an opponent reference", () => {
    expect(describeMatchEvent("houseReachedCastle", "player")).toBe("🏰 あなたの家がcastleまで発展した");
    expect(describeMatchEvent("houseReachedCastle", "enemy")).toBe("🏰 敵の家がcastleまで発展した");
  });
});
