...
function flatHeightmap(width: number, height: number, elevation: number): Heightmap {
  ...
  return { width, height, terrain: "grass", vertices, rockHardness, forest: blankLayer(width, height),
      crevice: blankLayer(width, height), scorched: blankLayer(width, height), road: blankLayer(width, height), fungus: blankLayer(width, height), wall: blankLayer(width, height), waterLevel: 0 };
}
...
describe("Simulation", () => {
  ...
  it("grows population slower on harsher terrain (rock) than on grass, all else equal", () => {
    ...
    const flatHeightmap = (terrain: Heightmap["terrain"]): Heightmap => ({
      ...
      crevice: blankLayer(width, height), scorched: blankLayer(width, height), road: blankLayer(width, height), fungus: blankLayer(width, height), wall: blankLayer(width, height),
      waterLevel: 0,
    });
