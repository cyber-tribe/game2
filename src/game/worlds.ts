...
import type { HeroKind } from "./components";
...
export interface WorldDefinition {
  ...
  /**
   * Whether 救出 works here — 「溺れた人間の救出」, the original's own
   * per-stage ○×.
   *
   * × on all fifteen of the first stages and then largely ○ from No.16 on,
   * with two later exceptions (No.26 and No.39). That shape is the point:
   * the early campaign is where water is simply lethal, and the operation
   * arrives as the campaign turns from "learn the spade" toward the long
   * middle where the enemy's 洪水 and 津波 are what you are living with.
   * Being able to pull your own people out of the sea is a mid-game
   * courtesy that can be taken away again.
   */
  rescueAllowed: boolean;
  /**
   * Whether the world map shows the enemy at all — 「敵の位置表示」, the
   * original's own per-stage ○×.
   *
   * × on eleven stages. This is not a handicap on what the player can *do*
   * — the enemy is still there, still visible on the ground in front of
   * you — it is a handicap on what they can *know* without going to look.
   * 「表示を奪うことが難易度の軸になっている」, and it is the cheapest
   * difficulty the original has: nothing about the opponent changes, only
   * how much of it arrives for free.
   */
  enemyPositionsVisible: boolean;
  /**
   * Whether a miracle the enemy casts is marked on the world map —
   * 「災害箇所表示」, the original's own per-stage ○×.
   *
   * × on six stages, every one of which also hides the enemy
   * (enemyPositionsVisible). The two are dealt as a pair at the hard end:
   * a stage that tells you neither where the enemy is nor where it just
   * struck is one you play by patrolling.
   */
  disasterMarkersVisible: boolean;
  /**
   * What the enemy god actually casts here — 資料の面ごとの「敵の神技」欄
   * (`docs/original-maps.md`).
   *
   * **Not `allowedMiracles`.** That is the *player's* hand; the guide lists
   * the god's own repertoire separately, and the two are routinely
   * different. No.1 deals the player 火柱・ペルセウス・アーマゲドン and
   * says of its god only 「土地上下とマグネットのみ。しかも反応が極めて
   * 遅い」 — the first stage is gentle because of the god, not the hand.
   * game2 read the player's hand to the enemy for a long time, which left
   * seven stages with a god that could cast nothing at all (see
   * `plan/0166`).
   *
   * Empty means exactly that: a god that throws no miracles.
   */
  enemyMiracles: readonly MiracleId[];
  /**
   * Which hero this god turns its leader into — 「ヒーローはアドニス」
   * 「特にペルセウスを多用」 and the like, named per stage by the guide.
   *
   * Undefined where the guide names none; the enemy then falls back on
   * ペルセウス／守護者化 the way it always has. game2's own 守護者 is never
   * named by the guide (the original has no defensive hero), so it stays a
   * fallback rather than a transcribed choice.
   */
  enemyHero?: HeroKind;
  /**
   * Multiplies this god's decision interval — the guide's own remarks about
   * *how often* it acts, which is a separate axis from what it holds:
   * 「執拗に仕掛けてくる」 (No.37, below 1), 「ほとんど仕掛けてこない」
   * (No.23), 「まず使わない」 (No.19), 「反応が極めて遅い」 (No.1).
   *
   * Undefined means the ordinary pace for that point in the campaign
   * (difficultyStep).
   */
...
const STAGES: readonly StageDefinition[] = [
  { id: "argos-1", place: "アルゴス", god: "プロメテウス", chapter: "地上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "fire", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: true, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: [], enemyCastRate: 3, allowedMiracles: ["shrine", "firePillar", "perseus", "guardian", "armageddon"] },
  { id: "argos-2", place: "アルゴス", god: "プロメテウス", chapter: "地上編", stage: 2, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "fire", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: true, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar"], allowedMiracles: ["shrine", "swamp", "firePillar", "lightning", "perseus", "guardian", "armageddon"] },
  { id: "argos-3", place: "アルゴス", god: "プロメテウス", chapter: "地上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "fire", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: true, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["fireRain"], allowedMiracles: ["shrine", "earthquake", "swamp", "firePillar", "lightning", "perseus", "guardian", "fireRain", "armageddon"] },
  { id: "sparta-1", place: "スパルタ", god: "ヘルメス", chapter: "地上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "air", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: true, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["tornado"], allowedMiracles: ["shrine", "earthquake", "holyWater", "tornado", "odysseus", "fireRain", "armageddon"] },
  { id: "sparta-2", place: "スパルタ", god: "ヘルメス", chapter: "地上編", stage: 2, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "air", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["tornado"], allowedMiracles: ["shrine", "swamp", "holyWater", "tornado", "odysseus", "fireRain", "armageddon"] },
  { id: "sparta-3", place: "スパルタ", god: "ヘルメス", chapter: "地上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "air", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["storm"], allowedMiracles: ["shrine", "lightning", "storm", "odysseus", "armageddon"] },
  { id: "crete-1", place: "クレタ", god: "ステュクス", chapter: "地上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "water", enemyTerritoryEditable: true, bottomlessSwamp: false, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["swamp"], allowedMiracles: ["shrine", "swamp", "firePillar", "adonis", "forest", "fireRain", "whirlpool", "armageddon"] },
  { id: "crete-2", place: "クレタ", god: "ステュクス", chapter: "地上編", stage: 2, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "water", enemyTerritoryEditable: true, bottomlessSwamp: false, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["swamp", "whirlpool"], allowedMiracles: ["shrine", "earthquake", "swamp", "firePillar", "road", "wall", "adonis", "forest", "armageddon"] },
  { id: "crete-3", place: "クレタ", god: "ステュクス", chapter: "地上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "water", enemyTerritoryEditable: true, bottomlessSwamp: false, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["swamp", "holyWater"], allowedMiracles: ["shrine", "earthquake", "swamp", "firePillar", "road", "wall", "fungus", "adonis", "forest", "flower", "armageddon"] },
  { id: "cyprus-1", place: "キプロス", god: "アフロディーテ", chapter: "地上編", stage: 1, terrain: "desert", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["storm", "holyWater"], allowedMiracles: ["shrine", "earthquake", "firePillar", "road", "wall", "megalith", "fungus", "hercules", "forest", "flower", "armageddon"] },
  { id: "cyprus-2", place: "キプロス", god: "アフロディーテ", chapter: "地上編", stage: 2, terrain: "desert", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["holyWater", "tornado", "fireRain"], allowedMiracles: ["shrine", "earthquake", "firePillar", "reef", "road", "wall", "megalith", "fungus", "hercules", "forest", "flower", "whirlpool", "armageddon"] },
  { id: "cyprus-3", place: "キプロス", god: "アフロディーテ", chapter: "地上編", stage: 3, terrain: "desert", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["holyWater", "storm", "hurricane"], allowedMiracles: ["shrine", "holyWater", "firePillar", "lightning", "hercules", "whirlpool", "armageddon"] },
  { id: "delos-1", place: "デロス", god: "アレス", chapter: "地上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["tornado", "firePillar"], allowedMiracles: ["shrine", "reef", "helen", "tsunami"] },
  { id: "delos-2", place: "デロス", god: "アレス", chapter: "地上編", stage: 2, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "tornado", "plague"], allowedMiracles: ["shrine", "earthquake", "swamp", "tornado", "firePillar", "reef", "road", "wall", "megalith", "fungus", "helen", "forest", "flower", "fireRain", "tsunami", "whirlpool"] },
  { id: "delos-3", place: "デロス", god: "アレス", chapter: "地上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "storm", "volcano", "tsunami"], allowedMiracles: ["shrine", "earthquake", "swamp", "holyWater", "lightning", "storm", "plague", "hurricane", "road", "wall", "megalith", "fungus", "helen", "forest", "flower", "fireRain", "whirlpool"] },
  { id: "athens-1", place: "アテナイ", god: "アテナ", chapter: "地上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar", "fungus", "holyWater", "plague", "swamp", "tornado", "volcano", "whirlpool"], enemyHero: "achilles", allowedMiracles: ["shrine", "earthquake", "swamp", "holyWater", "tornado", "firePillar", "plague", "reef", "road", "wall", "megalith", "fungus", "achilles", "flower", "volcano", "whirlpool", "armageddon"] },
  { id: "athens-2", place: "アテナイ", god: "アテナ", chapter: "地上編", stage: 2, terrain: "grass", terrainEditRule: "raiseOnly", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar", "fungus", "swamp", "tornado", "whirlpool"], allowedMiracles: ["shrine", "swamp", "tornado", "firePillar", "megalith", "fungus", "forest", "flower", "whirlpool"] },
  { id: "athens-3", place: "アテナイ", god: "アテナ", chapter: "地上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: true, enemyMiracles: ["holyWater", "lightning", "storm", "swamp"], enemyHero: "achilles", allowedMiracles: ["shrine", "swamp", "holyWater", "lightning", "storm", "wall", "achilles", "forest", "flower", "volcano", "tsunami", "armageddon"] },
  { id: "mycenae-1", place: "ミュケナイ", god: "ディオニュソス", chapter: "地上編", stage: 1, terrain: "desert", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: true, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: true, enemyMiracles: ["tsunami"], enemyHero: "adonis", enemyCastRate: 4, allowedMiracles: ["shrine", "holyWater", "road", "wall", "adonis", "flower", "armageddon"] },
  { id: "mycenae-2", place: "ミュケナイ", god: "ディオニュソス", chapter: "地上編", stage: 2, terrain: "desert", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "plant", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar", "fireRain", "swamp", "tornado", "tsunami", "volcano", "whirlpool"], allowedMiracles: ["shrine", "earthquake", "swamp", "tornado", "firePillar", "reef", "adonis", "fireRain", "volcano", "tsunami", "whirlpool", "armageddon"] },
  { id: "mycenae-3", place: "ミュケナイ", god: "ディオニュソス", chapter: "地上編", stage: 3, terrain: "desert", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "plant", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "hurricane", "plague", "tsunami", "volcano"], allowedMiracles: ["shrine", "earthquake", "plague", "hurricane", "megalith", "adonis", "flower", "volcano", "tsunami", "armageddon"] },
  { id: "larissa-1", place: "ラリッサ", god: "ヘパイストス", chapter: "地上編", stage: 1, terrain: "rock", terrainEditRule: "raiseOnly", enemyPersonality: "aggressive", enemySchool: "fire", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar"], enemyHero: "achilles", allowedMiracles: ["shrine", "holyWater", "firePillar", "reef", "achilles", "forest", "flower", "fireRain", "whirlpool"] },
  { id: "larissa-2", place: "ラリッサ", god: "ヘパイストス", chapter: "地上編", stage: 2, terrain: "rock", terrainEditRule: "neither", enemyPersonality: "aggressive", enemySchool: "fire", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar", "fungus", "tornado", "whirlpool"], enemyCastRate: 4, allowedMiracles: ["shrine", "tornado", "firePillar", "megalith", "fungus", "forest", "flower", "whirlpool"] },
  { id: "larissa-3", place: "ラリッサ", god: "ヘパイストス", chapter: "地上編", stage: 3, terrain: "rock", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "fire", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar", "fireRain", "tornado", "plague"], allowedMiracles: ["shrine", "hurricane", "reef", "megalith", "fungus", "armageddon"] },
  { id: "thrace-1", place: "トラキア", god: "デメテル", chapter: "地上編", stage: 1, terrain: "snow", terrainEditRule: "lowerOnly", enemyPersonality: "balanced", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["storm", "swamp", "holyWater"], enemyHero: "adonis", allowedMiracles: ["earthquake", "swamp", "holyWater", "lightning", "storm", "plague", "megalith", "adonis", "forest", "flower", "volcano", "whirlpool", "armageddon"] },
  { id: "thrace-2", place: "トラキア", god: "デメテル", chapter: "地上編", stage: 2, terrain: "snow", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: false, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar"], enemyHero: "adonis", allowedMiracles: ["shrine", "earthquake", "firePillar", "road", "wall", "adonis", "forest", "flower"] },
  { id: "thrace-3", place: "トラキア", god: "デメテル", chapter: "地上編", stage: 3, terrain: "snow", terrainEditRule: "raiseOnly", enemyPersonality: "balanced", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["tsunami"], enemyHero: "adonis", allowedMiracles: ["shrine", "reef", "adonis", "tsunami"] },
  { id: "olympus-1", place: "オリュンポス山", god: "ガイア", chapter: "地上編", stage: 1, terrain: "rock", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "earth", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake"], enemyHero: "hercules", allowedMiracles: ["shrine", "earthquake", "tornado", "firePillar", "plague", "hurricane", "reef", "road", "wall", "hercules", "forest", "flower", "fireRain", "volcano", "tsunami", "whirlpool", "armageddon"] },
  { id: "olympus-2", place: "オリュンポス山", god: "ガイア", chapter: "地上編", stage: 2, terrain: "rock", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "earth", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["hurricane"], enemyHero: "hercules", allowedMiracles: ["shrine", "hurricane", "hercules", "tsunami", "armageddon"] },
  { id: "olympus-3", place: "オリュンポス山", god: "ガイア", chapter: "地上編", stage: 3, terrain: "rock", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "earth", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake"], allowedMiracles: ["shrine", "lightning", "wall", "hercules", "flower", "armageddon"] },
  { id: "artemis-1", place: "月神の宮", god: "アルテミス", chapter: "天上編", stage: 1, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["plague"], enemyHero: "perseus", allowedMiracles: ["shrine", "plague", "perseus", "guardian", "armageddon"] },
  { id: "artemis-2", place: "月神の宮", god: "アルテミス", chapter: "天上編", stage: 2, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "plant", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["plague"], allowedMiracles: ["shrine", "swamp"] },
  { id: "artemis-3", place: "月神の宮", god: "アルテミス", chapter: "天上編", stage: 3, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "plant", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar", "fireRain", "swamp", "tornado", "tsunami", "volcano", "whirlpool"], enemyHero: "perseus", allowedMiracles: ["shrine", "earthquake", "swamp", "tornado", "firePillar", "reef", "perseus", "guardian", "flower", "fireRain", "volcano", "tsunami", "whirlpool", "armageddon"] },
  { id: "apollo-1", place: "太陽神の宮", god: "アポロン", chapter: "天上編", stage: 1, terrain: "desert", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "fire", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["fireRain", "firePillar", "volcano"], enemyHero: "achilles", allowedMiracles: ["shrine", "holyWater", "plague", "flower", "armageddon"] },
  { id: "apollo-2", place: "太陽神の宮", god: "アポロン", chapter: "天上編", stage: 2, terrain: "desert", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "fire", enemyTerritoryEditable: false, bottomlessSwamp: false, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar", "fireRain", "fungus", "holyWater", "hurricane", "lightning", "plague", "storm", "swamp", "tsunami", "volcano", "whirlpool"], enemyHero: "achilles", allowedMiracles: ["shrine", "earthquake", "swamp", "holyWater", "firePillar", "lightning", "storm", "plague", "hurricane", "reef", "road", "wall", "fungus", "achilles", "forest", "flower", "fireRain", "volcano", "tsunami", "whirlpool", "armageddon"] },
  ...
  { id: "hera-1", place: "天界王妃の宮", god: "ヘラ", chapter: "天上編", stage: 1, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["storm"], enemyCastRate: 0.5, allowedMiracles: ["shrine", "holyWater", "lightning", "plague", "odysseus"] },
  { id: "hera-2", place: "天界王妃の宮", god: "ヘラ", chapter: "天上編", stage: 2, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["storm", "firePillar"], allowedMiracles: ["shrine", "firePillar", "storm", "megalith", "fungus", "odysseus", "forest", "flower", "whirlpool", "armageddon"] },
  { id: "hera-3", place: "天界王妃の宮", god: "ヘラ", chapter: "天上編", stage: 3, terrain: "grass", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "human", enemyTerritoryEditable: true, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: false, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["fireRain", "volcano"], enemyHero: "odysseus", allowedMiracles: ["shrine", "swamp", "fungus", "odysseus", "forest", "flower"] },
  { id: "poseidon-1", place: "海王の宮", god: "ポセイドン", chapter: "天上編", stage: 1, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "water", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["earthquake", "firePillar", "fireRain", "fungus", "holyWater", "hurricane", "lightning", "plague", "storm", "swamp", "tsunami", "volcano", "whirlpool"], enemyHero: "helen", allowedMiracles: ["shrine", "earthquake", "swamp", "holyWater", "firePillar", "lightning", "storm", "plague", "hurricane", "reef", "road", "wall", "megalith", "fungus", "helen", "forest", "flower", "fireRain", "volcano", "tsunami", "whirlpool"] },
  { id: "poseidon-2", place: "海王の宮", god: "ポセイドン", chapter: "天上編", stage: 2, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "water", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["earthquake", "firePillar", "fireRain", "fungus", "holyWater", "hurricane", "plague", "swamp", "tornado", "tsunami", "volcano", "whirlpool"], enemyHero: "helen", allowedMiracles: ["shrine", "earthquake", "swamp", "holyWater", "tornado", "firePillar", "plague", "hurricane", "reef", "road", "wall", "megalith", "fungus", "helen", "forest", "flower", "fireRain", "volcano", "tsunami", "whirlpool"] },
  { id: "poseidon-3", place: "海王の宮", god: "ポセイドン", chapter: "天上編", stage: 3, terrain: "snow", terrainEditRule: "both", enemyPersonality: "defensive", enemySchool: "water", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["storm", "volcano"], enemyHero: "helen", allowedMiracles: ["shrine", "earthquake", "swamp", "firePillar", "lightning", "storm", "hurricane", "reef", "road", "wall", "megalith", "fungus", "helen", "forest", "flower", "fireRain", "volcano"] },
  { id: "hades-1", place: "冥王の宮", god: "ハデス", chapter: "天上編", stage: 1, terrain: "rock", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "earth", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: true, enemyMiracles: ["earthquake"], enemyHero: "perseus", allowedMiracles: ["shrine", "earthquake", "perseus", "guardian", "volcano"] },
  { id: "hades-2", place: "冥王の宮", god: "ハデス", chapter: "天上編", stage: 2, terrain: "rock", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "earth", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["firePillar", "fireRain"], allowedMiracles: ["shrine", "swamp", "fungus", "achilles", "forest", "flower"] },
  { id: "hades-3", place: "冥王の宮", god: "ハデス", chapter: "天上編", stage: 3, terrain: "rock", terrainEditRule: "both", enemyPersonality: "aggressive", enemySchool: "earth", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: true, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["holyWater", "firePillar", "hurricane", "volcano", "tsunami"], enemyHero: "hercules", allowedMiracles: ["shrine", "firePillar", "lightning", "road", "hercules", "forest"] },
  { id: "zeus-1", place: "オリュンポス神殿", god: "ゼウス", chapter: "天上編", stage: 1, terrain: "rock", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "air", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: false, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["earthquake", "firePillar", "fireRain", "lightning", "storm", "swamp", "tsunami", "volcano", "whirlpool"], allowedMiracles: ["shrine", "earthquake", "swamp", "firePillar", "lightning", "storm", "reef", "adonis", "fireRain", "volcano", "tsunami", "whirlpool", "armageddon"] },
  { id: "zeus-2", place: "オリュンポス神殿", god: "ゼウス", chapter: "天上編", stage: 2, terrain: "rock", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "air", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: false, rescueAllowed: true, enemyPositionsVisible: true, disasterMarkersVisible: true, enemyMiracles: ["holyWater", "tsunami", "whirlpool"], enemyHero: "odysseus", allowedMiracles: ["shrine", "lightning", "storm", "hurricane", "odysseus"] },
  { id: "zeus-3", place: "オリュンポス神殿", god: "ゼウス", chapter: "天上編", stage: 3, terrain: "rock", terrainEditRule: "both", enemyPersonality: "balanced", enemySchool: "air", enemyTerritoryEditable: false, bottomlessSwamp: true, openTerraforming: false, sprogAllowed: false, rescueAllowed: true, enemyPositionsVisible: false, disasterMarkersVisible: false, enemyMiracles: ["earthquake", "fireRain", "fungus", "holyWater", "hurricane", "lightning", "storm", "volcano"], enemyHero: "hercules", allowedMiracles: ["earthquake", "holyWater", "lightning", "storm", "hurricane", "reef", "wall", "fungus", "hercules", "forest", "flower", "fireRain", "volcano"] },
];
...
function buildWorlds(): WorldDefinition[] {
  return STAGES.map((stage, index) => ({
    ...
    rescueAllowed: stage.rescueAllowed,
    enemyPositionsVisible: stage.enemyPositionsVisible,
    disasterMarkersVisible: stage.disasterMarkersVisible,
    enemyMiracles: stage.enemyMiracles,
    enemyHero: stage.enemyHero,
    enemyCastRate: stage.enemyCastRate,
